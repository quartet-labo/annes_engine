...
class AnnesLoyalty::RewardRedemptionIssuerTest < AnnesLoyalty::TestCase
  test "issues a redemption token without storing the raw token" do
    ...
    AnnesLoyalty.earn!(member:, location:, amount_cents: 2_000, source: { type: "Receipt", key: "R-001" })

    issue = AnnesLoyalty.redeem_reward!(member:, reward:, actor: Account.create!(email: "customer@example.com"))
    ...
    assert_equal AnnesLoyalty::RedemptionToken.digest(issue.token), redemption.token_digest
  ...
  test "rejects inactive rewards and insufficient balances" do
    ...
    assert_raises(AnnesLoyalty::InactiveRewardError) do
      AnnesLoyalty.redeem_reward!(member:, reward:)
    end
  end
end
