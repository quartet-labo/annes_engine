...
AnnesLoyalty is a Rails engine for reusable loyalty points, rewards, and redemption workflows.

The engine owns point consistency: programs, locations, members, append-only ledger entries, point lots, rewards, redemptions, idempotency, and redemption token verification. Host applications own industry-specific workflows and screens, and should call `AnnesLoyalty.*` service APIs instead of directly mutating engine records.
...
```ruby
# config/initializers/annes_loyalty.rb
...
```ruby
AnnesLoyalty.enroll!(program:, owner:, member_key:)
AnnesLoyalty.quote_earn(member:, location:, amount_cents:, occurred_at: Time.current, context: {})
AnnesLoyalty.earn!(member:, location:, amount_cents:, source:, occurred_at: Time.current, actor: nil)
AnnesLoyalty.balance_for(member:)
AnnesLoyalty.redeem_reward!(member:, reward:, actor: nil)
AnnesLoyalty.confirm_redemption!(token:, location:, actor: nil)
AnnesLoyalty.reverse!(ledger_entry:, reason:, actor: nil)
```

These APIs are implemented by command services under `app/services/annes_loyalty`.

## Data Model

AnnesLoyalty stores reusable loyalty primitives under `annes_loyalty_loyalty_*`
tables:
...
2. Update `lib/annes_loyalty/version.rb` to the version you want to publish.
3. Update `CHANGELOG.md` and `UPGRADING.md` for that version.
4. Update the dependent example app lockfile by running `bundle update annes_loyalty`
...
6. Run the `Publish Gems` workflow with the `workflow_dispatch` trigger,
   selecting the `main` ref, `gem` = `annes_loyalty`, and `version` equal to
   `AnnesLoyalty::VERSION`.
...
8. Update host applications with `bundle update annes_loyalty` and run their full test suites.

The workflow fails before publishing if the `version` input does not match
`AnnesLoyalty::VERSION`, the changelog section is missing or empty, or the remote
