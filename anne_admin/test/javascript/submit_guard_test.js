...
class Form {
  constructor({ guarded = true, controls = [] } = {}) {
    ...
    if (guarded) this.setAttribute("data-anne-admin-submit-guard", "true");
  }
  ...
  matches(selector) {
    return selector === "[data-anne-admin-submit-guard]" &&
      this.hasAttribute("data-anne-admin-submit-guard");
  }
...
test("allows the first submit and disables controls after event processing", () => {
  ...
  assert.equal(
    submitter.getAttribute("data-anne-admin-submit-guard-disabled"),
    "true"
  );
});
