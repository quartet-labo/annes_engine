...
class AnneAdmin::PackageBoundaryTest < AnneAdmin::TestCase
  ...
  test "runtime files do not reference host application domain constants" do
    runtime_files = Dir[
      AnneAdmin::Engine.root.join("app/**/*.rb"),
      AnneAdmin::Engine.root.join("lib/anne_admin/**/*.rb")
    ...
    violations = runtime_files.filter_map do |path|
      ...
      Pathname(path).relative_path_from(AnneAdmin::Engine.root).to_s
    end
