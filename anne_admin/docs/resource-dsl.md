...
Define one host model per resource file. AnneAdmin loads Ruby files under
`app/admin/resources/**/*.rb` and `config/anne_admin/resources/**/*.rb` in
sorted path order.
...
`AnneAdmin::ConfigurationError`. Prefix filenames when navigation order matters.
...
```ruby
AnneAdmin.resource :audit_entries, model: "AuditEntry" do
  actions :index, :show
end
```
...
Unknown types raise `AnneAdmin::ConfigurationError` while loading the resource.
...
When omitted, AnneAdmin infers the list from permitted, non-readonly fields.
...
If no block is supplied and the relation responds to the scope name, AnneAdmin
...
response, AnneAdmin redirects to the member show or collection index page.
