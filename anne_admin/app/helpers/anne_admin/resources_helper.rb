module AnneAdmin
  module ResourcesHelper
    def anne_admin_resource_value(record, field)
      field.format(record)
    end

    def anne_admin_sort_link(resource, field)
      ...
      link_to field.label_text, anne_admin_resource_index_path(resource, request.query_parameters.merge(sort: field.name, direction: next_direction, page: nil)), class: "underline decoration-slate-300 underline-offset-2"
    end

    def anne_admin_record_title(record)
    ...
    def anne_admin_form_input(form, field)
      ...
      when :select
        form.select field.name, anne_admin_field_options(field), { include_blank: true }, html_options
      when :date_field
    ...
    def anne_admin_field_options(field)
      ...
      if collection.is_a?(Hash)
        ...
        collection.map do |item|
          if item.is_a?(Array)
            ...
            [ anne_admin_record_title(item), item.id ]
          else
            [ item.to_s, item ]
          end
        end
