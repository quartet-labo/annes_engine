module AnnesAuth
  module Accounts
    class RegistrationsController < AnnesAuth::ApplicationController
      layout "annes_auth"
      ...
      private
        def build_registration_resources
          @account = AnnesAuth.configuration.account_class.new(account_params)
        end
        ...
        def save_registration
          ...
          AnnesAuth.configuration.account_class.transaction do
            @account.save!
            AnnesAuth.configuration.account_created(@account, self)
        ...
        def deliver_verification_mail
          _verification_token, plain_code = AnnesAuth.configuration.account_verification_token_class.issue_for(@account)
          AnnesAuth.configuration.account_mailer_class.with(account: @account, plain_code:).verification.deliver_now
        end
