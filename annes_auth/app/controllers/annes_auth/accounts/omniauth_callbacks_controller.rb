module AnnesAuth
  module Accounts
    class OmniauthCallbacksController < AnnesAuth::ApplicationController
      layout "annes_auth"

      def create
        unless AnnesAuth.configuration.google_oauth_configured?
        ...
        if result.success?
          ...
          AnnesAuth::AccountEvent.emit(
            ...
            provider: AnnesAuth::Accounts::GoogleAuthentication::PROVIDER
          )

          if result.profile_required?
            session[:google_profile_name] = result.profile_name if result.profile_name.present?
            redirect_to AnnesAuth.configuration.account_profile_path.call(self, result.account), notice: "Googleでログインしました。顧客情報を入力してください。"
          else
