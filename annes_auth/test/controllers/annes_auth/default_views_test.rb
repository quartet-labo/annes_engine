...
class AnnesAuth::DefaultViewsTest < ActionDispatch::IntegrationTest
  test "renders account authentication views from the engine" do
    get "/auth/login"
    assert_response :success
    assert_select "h1", "ログイン"
    assert_select "form[data-annes-auth-submit-guard=true]", count: 1
    assert_select "script[nonce]", count: 1 do |scripts|
      assert_includes scripts.first.text, "window.AnnesAuthSubmitGuard"
    end
