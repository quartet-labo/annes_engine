...
class AnnesAuth::Accounts::InvitationDeliveryTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    AnnesAuth.configuration.account_mailer_class_name =
      "AnnesAuth::Accounts::InvitationDeliveryTest::RecordingInvitationMailer"
  ...
  test "resending invalidates the previously delivered invitation" do
    first_result = AnnesAuth::Accounts::InvitationDelivery.call(@account)
    first_plain_token = RecordingInvitationMailer.deliveries.last.fetch(:plain_token)

    second_result = AnnesAuth::Accounts::InvitationDelivery.call(@account)
  ...
  test "reports delivery failures without exposing the plain token" do
    ...
    retry_result = AnnesAuth::Accounts::InvitationDelivery.call(@account)
  ...
  test "does not convert account event subscriber errors into delivery failures" do
    ...
    error = assert_raises(RuntimeError) do
      ActiveSupport::Notifications.subscribed(subscriber, AnnesAuth::AccountEvent::EVENT_NAME) do
        AnnesAuth::Accounts::InvitationDelivery.call(@account)
      end
    end
