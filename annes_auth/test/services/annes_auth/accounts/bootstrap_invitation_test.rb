...
class AnnesAuth::Accounts::BootstrapInvitationTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @original_after_account_bootstrapped = AnnesAuth.configuration.after_account_bootstrapped
    AnnesAuth.configuration.account_mailer_class_name =
      "AnnesAuth::Accounts::BootstrapInvitationTest::RecordingBootstrapMailer"
    AnnesAuth.configuration.after_account_bootstrapped = ->(_account) {}
  ...
  teardown do
    ...
    AnnesAuth.configuration.after_account_bootstrapped = @original_after_account_bootstrapped
    RecordingBootstrapMailer.reset!
  end

  test "creates an unverified account and sends an invitation when no active account exists" do
    result = AnnesAuth::Accounts::BootstrapInvitation.call(email: "initial@example.com")
    ...
    claim = AnnesAuth::BootstrapClaim.find_by!(purpose: AnnesAuth::BootstrapClaim::INITIAL_ACCOUNT_PURPOSE)
  ...
  test "does not create a bootstrap account when an active account already exists" do
    ...
    assert_empty AnnesAuth::BootstrapClaim.all
  end

  test "returns invalid account without leaving bootstrap records" do
    result = AnnesAuth::Accounts::BootstrapInvitation.call(email: "invalid-email")
  ...
  test "retries invitation delivery for the same bootstrap account after delivery failure" do
    ...
    failed_result = AnnesAuth::Accounts::BootstrapInvitation.call(email: "initial@example.com")
    ...
    retry_result = AnnesAuth::Accounts::BootstrapInvitation.call(email: "initial@example.com")
  ...
  test "rolls back the account and claim when the host bootstrap hook fails" do
    AnnesAuth.configuration.after_account_bootstrapped = ->(_account) { raise "role assignment failed" }

    error = assert_raises(RuntimeError) do
      AnnesAuth::Accounts::BootstrapInvitation.call(email: "initial@example.com")
    end
  ...
  test "propagates validation failures raised by the host bootstrap hook" do
    AnnesAuth.configuration.after_account_bootstrapped = ->(account) {
  ...
  test "concurrent bootstrap creates only one initial account" do
    ...
    threads = 2.times.map do |index|
      Thread.new do
        ActiveRecord::Base.connection_pool.with_connection do
          ready << true
          start.pop
          results << AnnesAuth::Accounts::BootstrapInvitation.call(email: "initial-#{index}@example.com")
    ...
    assert_equal 1, CustomerAccount.count
    assert_equal 1, AnnesAuth::BootstrapClaim.count
