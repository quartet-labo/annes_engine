...
const partialPath = path.join(
  __dirname,
  "../../app/views/layouts/annes_auth/_submit_guard.html.erb"
);
...
class Form {
  constructor({ guarded = true, controls = [] } = {}) {
    ...
    if (guarded) this.setAttribute("data-annes-auth-submit-guard", "true");
  }
  ...
  matches(selector) {
    return selector === "[data-annes-auth-submit-guard]" &&
      this.hasAttribute("data-annes-auth-submit-guard");
  }
...
test("allows the first submit and disables controls after event processing", () => {
  ...
  assert.equal(
    submitter.getAttribute("data-annes-auth-submit-guard-disabled"),
    "true"
  );
});
