...
1. Confirm `AnnesAuth::Engine` is mounted in `config/routes.rb`.
...
Use the Engine route proxy or `AnnesAuth::Engine.routes.url_helpers` when calling
...
`annes_auth/db/migrate` before continuing. A skipped filename does not prove the
...
`AnnesAuth::Accounts::BootstrapInvitation` creates only the first active account.
...
`annes_auth_bootstrap_claims.completed_at` is set for `initial_account`.
...
to `annes_auth.account_event`. Events are emitted only for successful account
...
Model and association mappings are read when AnnesAuth model classes load.
