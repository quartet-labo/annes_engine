# AnnesAuth Security and Operations

Use this guide when preparing a host application for production. AnnesAuth
...
Successful authentication creates an `AnnesAuth::AccountSession` and a signed
...
Invitation responses set `Referrer-Policy: no-referrer`, and AnnesAuth adds
...
`AnnesAuth::Accounts::BootstrapInvitation.call(email:)` is for trusted setup
code only. It creates one unverified account with a random temporary password,
records an `annes_auth_bootstrap_claims` row for the `initial_account` purpose,
...
AnnesAuth emits `annes_auth.account_event` notifications for successful sign in,
...
AnnesAuth 1.0.0 does not implement MFA/TOTP runtime behavior. A future minimal
...
Credentials alone do not enable OAuth. AnnesAuth requires the feature flag,
...
`AnnesAuth::Account.active` excludes rows with `disabled_at`, and session loading
