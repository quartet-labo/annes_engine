(() => {
  const guard = window.AnnesAdminSubmitGuard ||= {};
  if (guard.installed) return;

  const formSelector = "[data-annes-admin-submit-guard]";
  const lockedAttribute = "data-annes-admin-submit-guard-locked";
  const disabledAttribute = "data-annes-admin-submit-guard-disabled";
