module AnnesAdmin
  class ResourceActionsController < ApplicationController
    ...
    def member
      ...
      authorize_annes_admin!(@action.name, record: @record)

      @action.call(record: @record, controller: self, resource: @resource)
      AuditEvent.emit(resource: @resource, action: @action.name, record: @record, user: annes_admin_current_user, status: :success)
    ...
    def collection
      ...
      authorize_annes_admin!(@action.name)

      @action.call(record: nil, controller: self, resource: @resource)
      AuditEvent.emit(resource: @resource, action: @action.name, record: nil, user: annes_admin_current_user, status: :success)
