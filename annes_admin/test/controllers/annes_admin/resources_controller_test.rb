...
class AnnesAdmin::ResourcesControllerTest < AnnesAdmin::IntegrationTest
  setup do
  ...
  test "shows configured action links when authorization is not configured" do
    ...
    assert_engine_selector "link[rel='stylesheet'][href*='annes_admin/application']"
    assert_engine_selector "a.annes-admin-action.annes-admin-action--primary[href='/admin/customers/new']"
    assert_engine_selector "input.annes-admin-action.annes-admin-action--primary[type='submit'][value='検索']"

    engine_get "/customers/#{customers(:anan).id}"

    assert_engine_response :success
    assert_includes engine_response.body, "編集"
    assert_engine_selector "a.annes-admin-action.annes-admin-action--secondary[href='/admin/customers']"
    assert_engine_selector "a.annes-admin-action.annes-admin-action--primary[href='/admin/customers/#{customers(:anan).id}/edit']"
  end
  ...
  test "styles form actions with engine action classes" do
    ...
    assert_engine_response :success
    assert_engine_selector "form[data-annes-admin-submit-guard='true']"
    assert_engine_selector "script[defer][src*='annes_admin/submit_guard']"
    assert_engine_selector "input.annes-admin-action.annes-admin-action--primary[type='submit'][value='保存']"
    assert_engine_selector "a.annes-admin-action.annes-admin-action--secondary[href='/admin/customers']"
    ...
    assert_engine_selector "script[defer][src*='annes_admin/submit_guard']"
    assert_engine_selector "input.annes-admin-action.annes-admin-action--primary[type='submit'][value='保存']"
    assert_engine_selector "a.annes-admin-action.annes-admin-action--secondary[href='/admin/customers/#{customers(:anan).id}']"
  end
  ...
  test "assigns resource-specific record variable" do
    controller = AnnesAdmin::ResourcesController.new
    controller.instance_variable_set(:@resource, AnnesAdmin.configuration.resources.fetch("customers"))
  ...
  test "allows host controllers to override resource variable names" do
    controller_class = Class.new(AnnesAdmin::ResourcesController) do
      private
  ...
  private
    def engine_session
      @engine_session ||= ActionDispatch::Integration::Session.new(AnnesAdmin::Engine)
    end
