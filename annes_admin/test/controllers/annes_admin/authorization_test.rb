...
class AnnesAdmin::AuthorizationTest < AnnesAdmin::IntegrationTest
  setup do
  ...
  test "returns forbidden when authorization hook rejects action" do
    ...
    get annes_admin.resource_index_path("customers")
  ...
  test "shows standard and member custom actions allowed by authorization" do
    ...
    assert_select "button.annes-admin-action.annes-admin-action--secondary", text: "Normalize"
  end
  ...
  test "keeps direct access to rejected actions forbidden" do
    ...
    get annes_admin.new_resource_path("customers")
    assert_response :forbidden

    get annes_admin.edit_resource_record_path("customers", customer)
    assert_response :forbidden
  ...
  test "uses the controller authorization context for action visibility" do
    customer = customers(:anan)
    resource = AnnesAdmin.configuration.resources.fetch("customers")
...

    {
      ...
      assert_instance_of AnnesAdmin::ResourcesController, context[:controller]
    end
  end
  ...
  test "does not let a view helper override bypass controller authorization" do
    ...
    assert_raises AnnesAdmin::NotAuthorizedError do
      controller.send(:authorize_annes_admin!, :edit, record: customers(:anan))
    end
  end
end
