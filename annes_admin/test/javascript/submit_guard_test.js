...
class Form {
  constructor({ guarded = true, controls = [] } = {}) {
    ...
    if (guarded) this.setAttribute("data-annes-admin-submit-guard", "true");
  }
  ...
  matches(selector) {
    return selector === "[data-annes-admin-submit-guard]" &&
      this.hasAttribute("data-annes-admin-submit-guard");
  }
...
test("allows the first submit and disables controls after event processing", () => {
  ...
  const submitter = new Control({ name: "commit", value: "保存" });
  ...
  assert.equal(submitter.value, "保存");
  ...
  assert.equal(
    submitter.getAttribute("data-annes-admin-submit-guard-disabled"),
    "true"
  );
});
