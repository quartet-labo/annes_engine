...
- Rename the published gem from `anne_admin` to `annes_admin` and the public
  Ruby namespace from `AnneAdmin` to `AnnesAdmin`.
- Rename the engine route helper, initializer, generator, assets, CSS classes,
  and audit notification from `anne_admin` / `anne-admin` to their
  `annes_admin` / `annes-admin` equivalents.
- This is a breaking release. See `UPGRADING.md` for the required host
  application changes.
