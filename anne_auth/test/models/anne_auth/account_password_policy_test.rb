...
class AnneAuth::AccountPasswordPolicyTest < ActiveSupport::TestCase
  setup do
    @original_minimum_length = AnneAuth.configuration.account_password_minimum_length
  end

  teardown do
    AnneAuth.configuration.account_password_minimum_length = @original_minimum_length
  end
  ...
  test "uses configured minimum length" do
    AnneAuth.configuration.account_password_minimum_length = 16
    account = CustomerAccount.new(
