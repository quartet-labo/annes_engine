...
class AnneAuth::AccountMailerTest < ActionMailer::TestCase
  ...
  test "invitation email contains the configured activation URL and expiry in both parts" do
    ...
    configured_calls = []
    original_account_invitation_url = AnneAuth.configuration.account_invitation_url
    AnneAuth.configuration.account_invitation_url = lambda do |mailer, token|
      configured_calls << [ mailer, token ]
      "https://accounts.example.test/auth/invitation?token=#{token}"
    end

    mail = AnneAuth::AccountMailer.with(account:, plain_token:).invitation
    ...
    assert_instance_of AnneAuth::AccountMailer, configured_calls.first.first
    ...
    AnneAuth.configuration.account_invitation_url = original_account_invitation_url
  end
end
