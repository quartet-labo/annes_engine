module AnneAudit
  class Recorder
    ...
    def initialize(source:, action:, actor: nil, target: nil, result: :success, request: nil, occurred_at: nil, metadata: {})
      @source = source
      @action = action
      @actor = actor
      @target = target
    ...
    private
      attr_reader :source, :action, :actor, :target, :result, :request, :occurred_at, :metadata
      ...
      def event_attributes
...

        {
          ...
          request_id: request_value(request_object, :request_id),
          ip_address: request_value(request_object, :remote_ip),
          user_agent: request_value(request_object, :user_agent),
