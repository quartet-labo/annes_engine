...
class AnneAudit::RecorderTest < AnneAudit::TestCase
  ...
  test "records actor target request and metadata" do
    ...
    assert_equal "AnneAudit::RecorderTest::Actor", event.actor_type
    assert_equal "account-123", event.actor_id
    assert_equal "admin@example.com", event.actor_label
    assert_equal "AnneAudit::RecorderTest::Target", event.target_type
  ...
  test "record returns false on validation failure" do
    assert_equal false, AnneAudit.record(source: nil, action: nil)
  end
end
