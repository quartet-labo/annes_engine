...
- metadata filtering;
- `AnneAudit.record!` and `AnneAudit.record`;
- generic notification mapper registration;
- built-in mappers for known Anne Engine notification contracts.

AnneAudit does not own authentication, authorization, admin CRUD screens, SIEM
...
```sh
bin/rails generate anne_audit:install
bin/rails db:migrate
```

## Data Model

`anne_audit_events` stores append-only event rows. Actor and target IDs are
...
```ruby
event = AnneAudit.record(source: "host", action: "customer.update")
```
...
```ruby
AnneAudit.with_context(actor: current_account, request: request) do
  CustomerUpdater.call(customer, params)
end
```

Any nested `AnneAudit.record!` call uses the context actor and request unless
explicit values are passed.

## Notification Mappers

AnneAudit can subscribe to any `ActiveSupport::Notifications` event through a
