module PreviewDefinition
  def create_preview_definition
    form = AnnesIntake::Form.create!(key: "preview", name: "Preview")
    version = form.versions.create!(number: 1, title: "お問い合わせプレビュー", description: "入力内容をご確認ください")
    AnnesIntake::TypeRegistry::WIDGETS.each do |type, widgets|
      widgets.each do |widget|
        ...
        field.update!(placeholder: "例：入力してください") if AnnesIntake::TypeRegistry::PLACEHOLDER_WIDGETS.include?(widget)
