...
class FlowDefinitionsTest < ActiveSupport::TestCase
  setup do
    form = AnnesIntake::Form.create!(key: "flow_form", name: "Contact")
    @form_version = form.versions.create!(number: 1, title: "Contact")
    @form_version.fields.create!(key: "name", label: "Name", required: true)
    AnnesIntake::Definitions::PublishVersion.call(@form_version, expected_lock_version: 0)
  ...
  test "published flow pins immutable steps and supports repeated form versions" do
    ...
    copy = AnnesIntake::Flows::Definitions::CloneVersion.call(@version)
    assert copy.draft?
  ...
  test "publishing validates forms and rolls back retirement on failure" do
    ...
    assert_raises(AnnesIntake::Flows::Error) { AnnesIntake::Flows::Definitions::PublishVersion.call(copy, expected_lock_version: 0) }
    assert @version.reload.published?
    assert copy.reload.draft?
  end

  test "stale editors and empty flows cannot publish" do
    assert_raises(ActiveRecord::StaleObjectError) { AnnesIntake::Flows::Definitions::DraftEditor.call(@version, expected_lock_version: 9) {} }
    @step.destroy!
    assert_raises(AnnesIntake::Flows::Error) { publish }
  end

  test "database rejects duplicate step keys and positions" do
    assert_raises(ActiveRecord::RecordNotUnique) do
      AnnesIntake::Step.transaction(requires_new: true) do
        AnnesIntake::Step.insert_all!([@step.attributes.except("id").merge("position" => 1)])
      end
    end
    assert_raises(ActiveRecord::RecordNotUnique) do
      AnnesIntake::Step.transaction(requires_new: true) do
        AnnesIntake::Step.insert_all!([@step.attributes.except("id").merge("key" => "different")])
      end
    end
  end
  ...
  private
    def publish
      AnnesIntake::Flows::Definitions::PublishVersion.call(@version, expected_lock_version: @version.reload.lock_version)
    end
end
