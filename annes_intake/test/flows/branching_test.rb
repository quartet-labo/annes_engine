...
class FlowBranchingTest < ActiveSupport::TestCase
  ...
  test "conditions use completed active answers and mapped values are read only" do
    ...
    assert_raises(AnnesIntake::Flows::Conflict) { save(@second, {"name" => "Tampered"}) }
    save(@second, {})
    complete(@second)
    finalize
  ...
  test "removed paths keep drafts exclude step_responses and require confirmation when restored" do
    ...
    assert_raises(AnnesIntake::Flows::Conflict) { AnnesIntake::Flows::FinalizeRun.call(run: @run, context: @context, token: stale) }
    save(@first, {"name" => "Restored", "options" => ["a"]}); complete(@first)
    assert @second.reload.draft?
    assert_equal "Restored", AnnesIntake::Flows::RouteEvaluator.raw_values(@second).fetch("name")
    save(@first, {"name" => "Final", "options" => []}); complete(@first)
    finalize
    assert_equal ["part_0"], @adapter.persisted.last.last.keys
    assert_nil @second.reload.step_response_id
  end
  ...
  test "cloning remaps conditions and mappings and published rules cannot be edited" do
    ...
    clone = AnnesIntake::Flows::Definitions::CloneVersion.call(@draft)
    ...
    assert_raises(AnnesIntake::Flows::Error) do
      target.condition_groups.first.conditions.first.update!(source_step: target)
      AnnesIntake::Flows::Definitions::Validator.call(clone)
    end
  end
  ...
  test "independent optional steps allow none either and both selections" do
    ...
    AnnesIntake::Flows::Definitions::PublishVersion.call(clone, expected_lock_version: 0)
    {[] => ["part_0"], ["a"] => %w[part_0 part_1], ["b"] => %w[part_0 other], %w[a b] => %w[part_0 part_1 other]}.each do |selection, keys|
      ...
      first = @run.step_runs.find_by!(step: source)
  ...
  test "boolean false is a value while missing and inactive sources do not match" do
    ...
    {nil => false, "0" => true, "false" => true, "true" => false}.each do |raw, active|
      result = AnnesIntake::Flows::RouteEvaluator.preview(clone, {"part_0" => {"name" => "A", "flag" => raw, "options" => ["a"]}})
      assert_equal active, result.steps.include?(target)
    end
    result = AnnesIntake::Flows::RouteEvaluator.preview(clone, {"part_0" => {"name" => "A", "flag" => "false", "options" => []}})
    ...
    assert_includes AnnesIntake::Flows::RouteEvaluator.preview(clone, {"part_0" => {"name" => "A", "options" => ["b"]}}).steps, target
  end
  ...
  test "publishing rejects foreign fields unknown options unsupported types and incompatible mappings" do
    ...
    [{expected_value: "unknown"}, {operator: "eq"}, {field: target.form_version.fields.first}, {source_step: target}].each do |attributes|
      ...
      assert_raises(AnnesIntake::Flows::Error) { AnnesIntake::Flows::Definitions::Validator.rules!(target) }
  ...
  test "inactive branch retains its text and file without copying them to the receipt" do
    ...
    assert second.reload.inactive?
    assert_equal "Retained text", AnnesIntake::Flows::DraftReader.call(second).fetch("note")
    finalize
    assert_nil second.reload.step_response_id
    assert_equal ["part_0"], @adapter.persisted.last.last.keys
    assert_equal "Retained branch file", blob.download
    assert_equal 0, AnnesIntake::AnswerAttachment.where(field_id: file.id).count
  ensure
    upload&.close!
  end

  private
    def path = AnnesIntake::Flows::RouteEvaluator.call(@run.reload)
    def token(action, step = nil) = AnnesIntake::Flows::OperationToken.issue(run: @run.reload, step: step, action: action, context: @context)
    def save(step, raw)
      AnnesIntake::Flows::SaveDraft.call(run: @run, step: step, context: @context, token: token(:save, step), raw_values: raw)
    end
    def complete(step)
      AnnesIntake::Flows::CompleteStep.call(run: @run, step: step, context: @context, token: token(:complete, step))
    end
    def finalize = AnnesIntake::Flows::FinalizeRun.call(run: @run, context: @context, token: token(:finalize))
end
