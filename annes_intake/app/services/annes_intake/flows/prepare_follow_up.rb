module AnnesIntake
  module Flows
    class PrepareFollowUp
      def self.call(root:, version:, context:, request_key:, title:, due_at:, custom: false, definition_context: context)
        ...
        Lock.call(root, extra_versions: [source]) do |current|
          ...
          raise Forbidden unless policy.scope(Run.where(id: current.id)).exists?
          ...
          raise Conflict unless (due_at.is_a?(Time) || due_at.is_a?(ActiveSupport::TimeWithZone)) && due_at.future?
          DefinitionPolicy.new(context: definition_context).authorize!(source, action: :admin_view_definition)
          source.steps.each { |step| DefinitionPolicy.new(context: definition_context).authorize!(step.form_version, action: :admin_view_definition) }
          source.reload
          ...
          if custom
            request.update!(definition_version: Definitions::CopyForFollowUp.call(source, request, context: context, definition_context: definition_context))
          end
          request
        end
      end
