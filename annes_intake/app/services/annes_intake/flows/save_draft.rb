    ...
    class SaveDraft
      ...
      def self.call(run:, step:, context:, token:, raw_values:, retained_attachments: {})
        Lock.call(run) do |current|
          ...
          mapped = owned_step.step.value_mappings.includes(:target_field).map { |mapping| mapping.target_field.key }
          ...
          current.step_runs.joins(:step).where("annes_intake_steps.position >= ?", owned_step.step.position).each { |item| item.update!(status: "draft") }
