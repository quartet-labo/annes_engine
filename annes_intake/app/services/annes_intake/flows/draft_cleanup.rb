module AnnesIntake
  module Flows
    class DraftCleanup
      def self.candidates
        Run.where(status: %w[cancelled expired]).or(Run.where(status: "in_progress").where("expires_at <= ?", Time.current))
      end

      def self.call
        candidates.find_each do |run|
          Lock.call(run) do |current|
            next current unless current.cancelled? || current.effective_expired?
