module AnnesIntake
  class AnswerReader
    def initialize(step_response)
      answers = step_response.association(:answers).loaded? ? step_response.answers : step_response.answers.includes(:field, options: :field_option, attachments: { file_attachment: :blob })
