      ...
      private
        def load_definition
          if params[:version_id]
            @version = definition_record(@definition_policy.scope(FormVersion.all).find(params[:version_id]))
          else
            @field = definition_record(@definition_policy.scope(Field.all).find(params[:id]))
            @version = definition_record(@field.form_version)
          end
        end
        ...
        def edit_draft(&block)
          Definitions::DraftEditor.call(@version, context: @definition_context, expected_lock_version: params[:lock_version], &block)
          @version.reload
        end
