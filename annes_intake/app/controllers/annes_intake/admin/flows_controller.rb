  ...
  module Admin
    class FlowsController < ActionController::Base
      ...
      def duplicate
        ...
        Flows::Definitions::CloneVersion.call(source, context: @definition_context)
        redirect_to admin_flow_path(flow), status: :see_other
      end
      def edit_version
        load_version
        Flows::Definitions::DraftEditor.call(@version, context: @definition_context, expected_lock_version: params[:lock_version]) do |draft|
          if params[:rule_step_id]
            ...
            if Condition.where(source_step_id: step.id).exists? || ValueMapping.where(source_step_id: step.id).exists?
              raise Flows::Error, "このステップを参照する条件・引継ぎを先に削除してください。"
            end
            step.destroy!
          elsif params[:step]
            attrs = params.require(:step).permit(:key, :title, :position, :form_version_id)
            definition_record(@definition_policy.scope(FormVersion.all).find(attrs[:form_version_id])) if attrs[:form_version_id]
            if params[:step_id]
      ...
      def publish
        load_version
        Flows::Definitions::PublishVersion.call(@version, context: @definition_context, expected_lock_version: params[:lock_version])
        redirect_to admin_flow_path(@flow), status: :see_other
      end
      def preview
        load_version
      ...
      private
        def load_version
          @flow = definition_record(@definition_policy.scope(Flow.all).find(params[:id]))
          @version = definition_record(@flow.versions.find(params[:version_id]))
        end
    end
  end
end
