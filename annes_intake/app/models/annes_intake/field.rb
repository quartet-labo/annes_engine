module AnnesIntake
  class Field < ApplicationRecord
    ...
    def assign_editor_defaults
      self.key = "field_#{SecureRandom.hex(6)}" if key.blank?
      self.position = (form_version.fields.maximum(:position) || -1) + 1 if position.nil?
    end

    include DraftDefinition

    TYPES = TypeRegistry::WIDGETS.keys.freeze
    belongs_to :form_version, class_name: "AnnesIntake::FormVersion", inverse_of: :fields
    has_many :options, -> { order(:position, :id) }, class_name: "AnnesIntake::FieldOption", dependent: :destroy, inverse_of: :field
    has_many :file_types, class_name: "AnnesIntake::FieldFileType", dependent: :destroy, inverse_of: :field
