...
class AnneAdmin::QueryTest < AnneAdmin::TestCase
  test "searches allowlisted attributes" do
    ...
    query = AnneAdmin::Query.new(resource, { q: customers(:anan).contact_name })
  ...
  test "ignores non-text searchable attributes" do
    ...
    query = AnneAdmin::Query.new(resource, { q: customers(:anan).id.to_s })
  ...
  test "ignores non-allowlisted sort params" do
    ...
    query = AnneAdmin::Query.new(resource, { sort: "email", direction: "desc" })
  ...
  test "caps pagination size" do
    AnneAdmin.configuration.max_per_page = 1
    resource = AnneAdmin::ResourceConfig.new(:customers, model: "Customer")
    query = AnneAdmin::Query.new(resource, { per_page: 500 })
  ...
  test "uses an injected base relation" do
    resource = AnneAdmin::ResourceConfig.new(:customers, model: "Customer")
    query = AnneAdmin::Query.new(resource, {}, relation: Customer.where(email: customers(:anan).email))
