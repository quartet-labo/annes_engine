...
class AnneAccess::PackageBoundaryTest < AnneAccess::TestCase
  ...
  test "runtime files do not reference host application domain constants" do
    runtime_files = Dir[
      AnneAccess::Engine.root.join("app/**/*.rb"),
      AnneAccess::Engine.root.join("lib/anne_access/**/*.rb")
    ...
    violations = runtime_files.filter_map do |path|
      ...
      Pathname(path).relative_path_from(AnneAccess::Engine.root).to_s
    end
