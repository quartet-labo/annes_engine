  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def copy_initializer
        template "initializer.rb", "config/initializers/anne_access.rb"
      end

      def copy_seed_example
        template "seeds.rb", "db/seeds/anne_access.rb"
      end

      def copy_migrations
        Dir.glob(File.expand_path("../../../db/migrate/*.rb", __dir__)).sort.each do |migration|
          migration_name = anne_access_migration_file_name(migration)

          if anne_access_migration_exists?(migration_name)
      ...
      private
        def anne_access_migration_file_name(path)
          File.basename(path).sub(/\A\d+_/, "")
        end

        def anne_access_migration_exists?(migration_name)
