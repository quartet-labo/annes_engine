# AnneAccess Record Scoping

AnneAccess answers whether a principal has an action for a resource. It does not
...
because `AnneAccess.can?` returned true.
...
relation. AnneAccess intentionally has no `accessible_by` equivalent.
