...
| --- | --- | --- |
| `AnneAccess::Role` | Named set of permissions | `key` is unique, lowercase, and limited to letters, digits, and underscores; `name` is required |
| `AnneAccess::Permission` | One resource/action pair | `resource` and `action` are normalized to lowercase; the pair and generated `key` are unique |
| `AnneAccess::RolePermission` | Connects a role to a permission | A permission can be connected to a role only once |
| `AnneAccess::Assignment` | Connects a polymorphic principal to a role | A principal can receive the same role only once |

The four Engine migrations create `anne_access_roles`,
`anne_access_permissions`, `anne_access_role_permissions`, and
`anne_access_assignments`. The host owns the rows and their lifecycle.
...
```ruby
AnneAccess::Permission.find_or_create_by!(
  resource: "invoices",
  action: "approve"
)
```
...
```ruby
AnneAccess.can?(account, :index, :customers)   # admin customers.manage => true
AnneAccess.can?(account, :destroy, :projects) # admin projects.manage => true
AnneAccess.can?(nil, :read, :customers)       # => false
AnneAccess.can?(account, :read, :unknown)     # => false
```
