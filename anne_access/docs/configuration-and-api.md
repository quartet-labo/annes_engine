# AnneAccess Configuration and API
...
AnneAccess.configure do |config|
  ...
  config.action_aliases = AnneAccess::Configuration::DEFAULT_ACTION_ALIASES.dup
...
`AnneAccess.reset_configuration!` restores these defaults and is useful in test
teardown.
...
```ruby
AnneAccess.can?(account, :index, :projects)   # projects.manage may grant this
AnneAccess.can?(account, :approve, :projects) # requires projects.approve
```
...
config.action_aliases =
  AnneAccess::Configuration::DEFAULT_ACTION_ALIASES.merge(export: :read)
```
...
```ruby
AnneAccess.can?(principal, action, resource, record: nil)
```
...
### `AnneAccess.authorize!`

```ruby
AnneAccess.authorize!(principal, :update, :projects, record: project)
```

Returns `true` when allowed and raises `AnneAccess::NotAuthorizedError` when
denied.

### `AnneAccess.ability_for`

```ruby
ability = AnneAccess.ability_for(account)
...
Do not rescue all `AnneAccess::Error` values as an allow or redirect loops may
