...
class AnnesAccess::PackageBoundaryTest < AnnesAccess::TestCase
  ...
  test "runtime files do not reference host application domain constants" do
    runtime_files = Dir[
      AnnesAccess::Engine.root.join("app/**/*.rb"),
      AnnesAccess::Engine.root.join("lib/annes_access/**/*.rb")
    ...
    violations = runtime_files.filter_map do |path|
      ...
      Pathname(path).relative_path_from(AnnesAccess::Engine.root).to_s
    end
