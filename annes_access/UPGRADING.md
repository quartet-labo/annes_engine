# AnnesAccess Upgrade Guide

This guide lists manual steps required when upgrading AnnesAccess in a host
...
2. Run `bundle update annes_access`.
3. Copy any new AnnesAccess migrations into the host app.
   - Preferred: run `bin/rails generate annes_access:install` and review
     generated / skipped files.
   - Alternative: copy only the missing migration files from
     `annes_access/db/migrate`.
4. Run `bin/rails db:migrate`.
5. Review `config/initializers/annes_access.rb`.
...
`1.0.0` is a breaking package, namespace, and database-prefix rename. Back up the database before applying migrations in production.

1. Replace `gem "anne_access"` with `gem "annes_access", "~> 1.0"`, then replace `require "anne_access"` and `AnneAccess` with `require "annes_access"` and `AnnesAccess`.
2. Rename `config/initializers/anne_access.rb` and `db/seeds/anne_access.rb` to their `annes_access` equivalents. Update host seed loaders and authorization integrations accordingly.
3. Run `bin/rails generate annes_access:install`, review the copied forward migration, then run `bin/rails db:migrate`. It renames `anne_access_roles`, `anne_access_permissions`, `anne_access_role_permissions`, and `anne_access_assignments` together with branded indexes. Existing rows and foreign keys are retained.
4. Run authorization, administration, and seed idempotency tests. Roll back only before application code starts writing through both naming versions.

## 0.1.x（旧package向けの未リリース履歴）

以下は `anne_access` 0.1.x の利用者向けに作成された履歴です。AnnesAccess
1.0.0 へ移行する場合は、先に上記の `0.1.x -> 1.0.0` 手順を完了してください。
