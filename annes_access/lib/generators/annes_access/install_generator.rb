require "rails/generators"
require "rails/generators/active_record/migration"

module AnnesAccess
  module Generators
    class InstallGenerator < Rails::Generators::Base
      include ActiveRecord::Generators::Migration

      source_root File.expand_path("templates", __dir__)

      def self.next_migration_number(dirname)
        previous_number = @previous_migration_number || Time.now.utc.strftime("%Y%m%d%H%M%S").to_i
        @previous_migration_number = [ current_migration_number(dirname), previous_number ].max + 1
        @previous_migration_number.to_s
      end

      def copy_initializer
        copy_template_unless_legacy_file(
          "initializer.rb",
          "config/initializers/annes_access.rb",
          "config/initializers/anne_access.rb"
        )
      end

      def copy_seed_example
        copy_template_unless_legacy_file(
          "seeds.rb",
          "db/seeds/annes_access.rb",
          "db/seeds/anne_access.rb"
        )
      end

      def copy_migrations
        Dir.glob(File.expand_path("../../../db/migrate/*.rb", __dir__)).sort.each do |migration|
          migration_name = annes_access_migration_file_name(migration)

          if annes_access_migration_exists?(migration_name)
            say_status :skip, "db/migrate/*_#{migration_name}.rb", :yellow
          else
            migration_template migration, "db/migrate/#{migration_name}"
          end
        end
      end

      private
        def copy_template_unless_legacy_file(template_name, destination, legacy_destination)
          if File.exist?(File.join(destination_root, legacy_destination))
            say_status :skip, legacy_destination, :yellow
            say "Rename #{legacy_destination} to #{destination} before using AnnesAccess.", :yellow
          else
            template template_name, destination
          end
        end

        def annes_access_migration_file_name(path)
          File.basename(path).sub(/\A\d+_/, "")
        end

        def annes_access_migration_exists?(migration_name)
          Dir.glob(File.join(destination_root, "db/migrate/*_#{migration_name}")).any?
        end
    end
  end
end
