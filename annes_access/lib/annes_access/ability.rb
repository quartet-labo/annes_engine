    ...
    private
      def allowed_by_role?(action, resource)
        ...
        Permission
          ...
          .where(annes_access_role_permissions: { role_id: role_ids })
          .where(resource:)
      ...
      def super_admin?
        super_admin_role_keys = Array(AnnesAccess.configuration.super_admin_role_keys).map(&:to_s)
      ...
      def default_role_id
        default_role_key = AnnesAccess.configuration.default_role_key
      ...
      def apply_custom_rule(principal:, action:, resource:, record:, allowed:)
        custom_rule = AnnesAccess.configuration.custom_rule
