module AnnesInquiry
  module Flows
    class RouteEvaluator
      ...
      def self.call(run)
        return run.step_runs.where.not(submission_id: nil).joins(:flow_step).order("annes_inquiry_flow_steps.position").to_a if run.submitted?
        evaluate(run).steps
      end

      def self.raw_values(step) = evaluate(step.flow_run).raw_values.fetch(step.flow_step_id, {})

      def self.evaluate(run)
        steps = run.step_runs.includes(:flow_step).sort_by { |step| step.flow_step.position }
        evaluate_steps(steps, definition: ->(step) { step.flow_step }, raw: ->(step) { DraftReader.call(step) }, complete: ->(step) { step.complete? })
      end
