module AnnesInquiry
  ...
  class NotificationDispatcher
    def self.call(id)
      ...
      request = FlowNotificationRequest.find_by(id: id)
    ...
    def self.recover!(stale_before: 1.hour.ago)
      FlowNotificationRequest.processing.where("processing_started_at < ?", stale_before)
        .update_all(status: "unknown", last_error: "Processing interrupted; delivery must be checked", updated_at: Time.current)
      FlowNotificationRequest.pending.find_each { |request| call(request.id) }
    end
  end
end
end
