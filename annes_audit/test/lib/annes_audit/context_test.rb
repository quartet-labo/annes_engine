...
class AnnesAudit::ContextTest < AnnesAudit::TestCase
  ...
  test "record uses context actor and request when omitted" do
    ...
    event = AnnesAudit.with_context(actor: actor, request: request) do
      AnnesAudit.record!(source: "host", action: "customer.update")
    end
  ...
  test "context is restored after the block" do
    ...
    AnnesAudit.with_context(actor: actor) do
      assert_equal actor, AnnesAudit::Context.current.fetch(:actor)
    end

    assert_empty AnnesAudit::Context.current
  end
end
