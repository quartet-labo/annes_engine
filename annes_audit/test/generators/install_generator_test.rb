...
require "generators/annes_audit/install_generator"

class AnnesAudit::InstallGeneratorTest < Rails::Generators::TestCase
  tests AnnesAudit::Generators::InstallGenerator
  destination Rails.root.join("tmp/generators/annes_audit_install")
  ...
  test "copies initializer and timestamped migrations" do
    ...
    assert_file "config/initializers/annes_audit.rb", /AnnesAudit.configure/
    assert_file "config/initializers/annes_audit.rb", /raise_on_persistence_error/
    assert_file "config/initializers/annes_audit.rb", /notification_subscribers.register/
    assert_migration "create_anne_audit_events.rb"
    assert_migration "rename_anne_audit_events_to_annes_audit_events.rb"
  end
  ...
  test "preserves an existing initializer when rerun" do
    run_generator
    initializer_path = File.join(destination_root, "config/initializers/annes_audit.rb")
    File.open(initializer_path, "a") { |file| file << "\n# host application customization\n" }

    run_generator

    initializer = File.read(initializer_path)
    assert_equal 1, initializer.scan("AnnesAudit.configure").length
    ...
    assert_equal 1, generated_migrations.count { |path| path.end_with?("_rename_anne_audit_events_to_annes_audit_events.rb") }
  end

  test "does not create a second initializer when the legacy initializer exists" do
    legacy_path = File.join(destination_root, "config/initializers/anne_audit.rb")
    FileUtils.mkdir_p File.dirname(legacy_path)
    File.write legacy_path, "AnneAudit.configure do; end\n"

    run_generator

    assert_file "config/initializers/anne_audit.rb", /AnneAudit.configure/
    assert_no_file "config/initializers/annes_audit.rb"
  end
