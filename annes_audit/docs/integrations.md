# AnnesAudit Integrations

AnnesAudit integrates with notification streams through mapper registration.
...
attributes accepted by `AnnesAudit.record!`.
...
The mapper lets `AnnesAudit::Event` generate `event_id`. Do not use
...
events. AnnesAudit still applies its own metadata filter before storage.
...
`AnnesAudit.record!` inside the domain transaction instead of relying on a
